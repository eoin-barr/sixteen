...
export async function getSessionFromCookie(token: string): Promise<SessionCookie> {
  let session: SessionCookie;

  try {
    session = await Iron.unseal(token, process.env.APP_SECRET as string, Iron.defaults);
  } catch (err) {
...
export async function createSessionCookie(res: Response, session: SessionCookie) {
  const createdAt = Date.now();
  const cook: SessionCookie = { ...session, createdAt, maxAge: MAX_AGE };
  const token = await Iron.seal(cook, process.env.APP_SECRET as string, Iron.defaults);
