...
type Post struct {
	...
	Content    string `binding:"required,min=3,max=1000"`
	CreatedAt  time.Time
...
func FetchUserPosts(user *User) error {
	err := db.Model(user).WherePK().Relation("Posts", func(q *orm.Query) (*orm.Query, error) {
		return q.Order("id ASC"), nil
	}).Select()
	if err != nil {
		log.Error().Err(err).Msg("Error fetching user posts")
	}
	return err
}
